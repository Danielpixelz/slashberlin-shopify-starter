...
exports.getShopify = async (req, res, next) => {
  // const token = req.user.tokens.find((token) => token.kind === 'shopify');
 ...
 * Post a product.
 */
exports.postShopifyProduct = async (req, res, next) => {
  const validationErrors = [];
  if (validator.isEmpty(req.body.product_title)) validationErrors.push({ msg: 'Product title cannot be empty' });

  if (validationErrors.length) {
    req.flash('errors', validationErrors);
    return res.redirect('/api/shopify');
  }
  ...
  try {
    const product = await shopify.product.create({ title: req.body.product_title });
      // if (err) { return next(err); }
      // req.flash('success', { msg: 'Your tweet has been posted.' });
      res.redirect('/api/shopify');
  } catch (error) {
    next(error);
  }
  ...
  // const token = req.user.tokens.find((token) => token.kind === 'shopify');
  // const T = new Twit({
  //   consumer_key: process.env.SHOPIFY_KEY,
  //   consumer_secret: process.env.SHOPIFY_SECRET,
